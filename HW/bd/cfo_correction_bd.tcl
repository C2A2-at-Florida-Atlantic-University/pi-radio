...
################################################################
# This is a generated script based on design: cfo_correction
#
...
# source cfo_correction_script.tcl


# The design that will be created by this Tcl script contains the following 
# module references:
# angle, axis_splitter, axis_splitter, complex_mult, conj, conj, cp_rm2, cp_rm, delay, delay, mux, scale, sum, tlast_symbol, conj, dds_top, mixer
...
proc create_root_design { parentCell } {
  ...
  # Create instance: tlast_symbol_0, and set properties
  set block_name tlast_symbol
  set block_cell_name tlast_symbol_0
  if { [catch {set tlast_symbol_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $tlast_symbol_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  connect_bd_intf_net -intf_net tlast_symbol_0_m_axis [get_bd_intf_pins mux_0/s_axis0] [get_bd_intf_pins tlast_symbol_0/m_axis]
